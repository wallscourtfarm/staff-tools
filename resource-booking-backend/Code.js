...
const PREFIX = 'booking_'; // key prefix to avoid collisions with other rows/keys
...
// cover-plan-state's and teaching-schedule-backend's migrations (see
// cover-plan-state's Code.js for the full story). One row per weekKey,
// column A = key, column B = JSON value. The backup ring buffer below stays
// on PropertiesService — it's small metadata, not the primary record.
const STATE_SHEET_ID = '1DBO1GERb_BRq-cnn1rousFtkVn9ajFnurzqitgknelc';
const STATE_TAB_NAME = 'Booking';
