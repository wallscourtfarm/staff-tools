#!/usr/bin/env bash
# Re-deploy the spellings-tracker backend after editing Code.js.
...
# caller, breaking every device's sync. See feedback_apps_script_deploy_
# anyone_anonymous in Claude's memory for how this was diagnosed (2026-09-08).

set -e
cd "$(dirname "$0")"

DEPLOYMENT_ID="AKfycbzwY-2zUMhFsNDdck_hgGswxXgOusGozaogPMR5w9-IWgCy4jmLuLcv88r2vs6OYpmW"
...
echo ""
echo "→ Verifying doGet/doPost is still publicly reachable (no sign-in wall)..."
sleep 3
...
if echo "$RESPONSE" | grep -q "accounts.google.com"; then
  ...
  echo "  ✓ Live and responding (check the body above manually if unsure)."
fi
